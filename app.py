...
st.download_button(
    ...
    mime="text/csv",
    help="Download the full trend summary as CSV"
)
...
with col1:
    ...
    pos_text = " ".join(df[df["sentiment"] > 0.3]["clean"].astype(str))
    if pos_text.strip() and len(pos_text.split()) > 10:
        wc = WordCloud(width=600, height=400, background_color="white", colormap="Greens", max_words=100).generate(pos_text)
...
with col2:
    ...
    if neg_text.strip() and len(neg_text.split()) > 10:
